...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Slipcord")
	var updateFlag = flag.Bool("repair", false, "Repair Slipcord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Slipcord")
	...
	if *versionFlag {
		fmt.Println("Slipped Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2026 Vendicated, thororen1234, Vencord, and Slipcord contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Slipped' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Slipcord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Slipcord will likely not work.")
}
